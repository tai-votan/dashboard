  ...
  return (
    <Link
      ...
      className={cn(
        ...
        'h-10 justify-start text-wrap rounded-none px-4 text-xs',
        { 'w-full px-2': subLink },
        className,
      )}
    ...
    <Collapsible defaultOpen={isChildActive}>
      <CollapsibleTrigger
        className={cn(
          ...
          'group h-10 w-full justify-start rounded-none px-4 text-xs',
        )}
      >
      ...
      <CollapsibleContent
        className="overflow-hidden data-[state='open']:animate-slide-down animate-slide-up"
        asChild
      >
        <ul>
          {sub!.map((sublink) => (
        ...
        {sub!.map(({ title, icon, label, href }) => (
          <DropdownMenuItem key={`${title}-${href}`} asChild>
            <Link
              href={href}
              className={cn({
                'bg-secondary': checkActiveNav(href),
              })}
            >
